...
public class foodValueText : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		Debug.Log(" old food ~~~> " + (GameManager.food - GameManager.oldFood));
		curFood = GameManager.food - GameManager.oldFood;

		if (curFood > 0)
		{
			transform.gameObject.GetComponent<SpriteText>().Text = "+Food " + curFood.ToString();
		}
		else
		{
