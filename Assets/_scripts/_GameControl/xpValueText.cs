...
public class xpValueText : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		//Debug.Log("@@--> " + scr_userLoad.ReturnXPcostTotal(scr_GameObjectSvr.objHound.objTypeId, transform.gameObject));



		transform.gameObject.GetComponent<SpriteText>().Text = "+XP" + " " + GameManager.earnXpVal.ToString();
		//transform.gameObject.GetComponent<SpriteText>().Text = "Indrajit";
	}
	...
	void Update () 
	{
		...
		{
			if (transform.root.gameObject.tag == "Plot")
			{
				GameObject foodValObj = Instantiate(Resources.Load("foodValue"), 
